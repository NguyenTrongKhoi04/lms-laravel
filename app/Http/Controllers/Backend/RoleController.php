...
class RoleController extends Controller
{
    ...
    public function AdminUpdateRoles(Request $request, $id)
    {
        ...
        // Kiểm tra và loại bỏ quyền không tồn tại
        $validPermissions = Permission::whereIn('id', $permissions)->pluck('id')->toArray();

        if (count($validPermissions) !== count($permissions)) {
            return back()->withErrors(['error' => 'Some permissions are invalid.']);
        }

        // Đồng bộ hóa quyền hợp lệ với role
        ...
        // dd($role->toArray(), $role->permissions->toArray());

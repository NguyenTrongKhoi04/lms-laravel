    ...
    if (p.type === 6) {
      ...
      if (p.name === "bind" && isStaticArgOf(p.arg, "name")) {
        if (p.exp) {
          ...
          const name = camelize2(p.arg.content);
  ...
  if (arg.type === 4) {
    if (arg.isStatic) {
      ...
      const eventString = node.tagType !== 0 || rawName.startsWith("vnode") || !/[A-Z]/.test(rawName) ? (
        ...
        toHandlerKey2(camelize2(rawName))
      ) : (
...
var capitalize3 = cacheStringFunction2((str) => {
  return str.charAt(0).toUpperCase() + str.slice(1);
});
var toHandlerKey3 = cacheStringFunction2((str) => {
...
var parserOptions = {
  ...
  isNativeTag: (tag) => isHTMLTag2(tag) || isSVGTag2(tag) || isMathMLTag2(tag),
...
var capitalize4 = cacheStringFunction3((str) => {
  return str.charAt(0).toUpperCase() + str.slice(1);
});
var toHandlerKey4 = cacheStringFunction3((str) => {
  const s = str ? `on${capitalize4(str)}` : ``;
  return s;
});
...
function getCache(options) {
  let c = compileCache.get(options != null ? options : EMPTY_OBJ3);
  if (!c) {
    c = /* @__PURE__ */ Object.create(null);
    compileCache.set(options != null ? options : EMPTY_OBJ3, c);
  }
  return c;
}
function compileToFunction(template, options) {
  ...
  function onError(err, asWarning = false) {
    ...
    const codeFrame = err.loc && generateCodeFrame2(
      template,
